...
![Kiro Chat 패널 위치 - 화면 오른쪽에 채팅창이 보입니다](../.gitbook/assets/placeholder-kiro-chat-panel-location.png)

> **잠깐! 💡**
> Kiro Chat 패널이 안 보이시나요?
> - 화면 오른쪽을 살펴보세요. "Chat" 또는 "Kiro" 라고 쓰인 탭이 있을 수 있습니다
> - 정말 안 보이면 진행자에게 손을 들어주세요 🙋

Chat 패널 하단에 **텍스트 입력창**이 보이시나요?
바로 거기에 한글로 원하는 것을 쓰면 됩니다!

![Kiro Chat 입력창 - 하단에 텍스트를 입력할 수 있는 영역](../.gitbook/assets/placeholder-kiro-chat-input-area.png)
...
![프롬프트를 입력한 모습 - Chat 입력창에 위 내용이 붙여넣어진 상태](../.gitbook/assets/placeholder-prompt-pasted-in-chat.png)
...
![AI가 코드를 생성하는 중 - 채팅창에 설명이 나오고 코드 영역에 코드가 자동으로 작성됩니다](../.gitbook/assets/placeholder-ai-generating-code.png)
...
![코드 생성 완료 - AI가 더 이상 작성하지 않고 멈춘 상태](../.gitbook/assets/placeholder-code-generation-complete.png)
...
![파일 탐색기에서 index.html 파일 찾기](../.gitbook/assets/placeholder-find-index-html.png)
...
2. 메뉴가 뜨면 **Open with Live Server** 를 찾아서 클릭합니다

![오른쪽 클릭 메뉴에서 Open with Live Server 선택](../.gitbook/assets/placeholder-right-click-open-live-server.png)

> **ℹ️ Live Server란?**
> 만든 앱을 **브라우저에서 바로 미리보기** 해주는 편리한 기능입니다.
> 이미 설치되어 있으니 걱정 마세요! 그냥 클릭만 하면 됩니다. 😊

### 4-3. 브라우저 확인

잠시 후 **브라우저(인터넷 창)가 자동으로 열리면서** 앱이 나타납니다!

![브라우저에 완성된 GS25 규정 도우미 앱이 표시된 모습](../.gitbook/assets/placeholder-app-in-browser.png)
...
2. **Live Server로 열었나요?**
   - 파일을 그냥 더블클릭한 게 아니라, **오른쪽 클릭 → Open with Live Server** 로 열었는지 확인하세요

3. **브라우저 주소를 확인해보세요**
   - 주소창에 `http://127.0.0.1:5500` 또는 `http://localhost:5500` 과 비슷한 주소가 보여야 합니다
   - 주소가 `file://`로 시작하면 → Live Server가 아닌 다른 방법으로 열린 겁니다. 다시 시도해주세요

4. 위 방법으로도 안 되면 → **진행자에게 문의해주세요** 🙋
